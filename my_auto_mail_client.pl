        ...
        if ($subject =~ m/\Q$ini_subject/ig )
        {
            ...
            if ($externReturncode ne 0) {
                if (defined $fail_email_address && defined $fail_email_body ) {
                    ...
                    $fail_email_subject = "$ini_subject ERROR " . $fail_email_subject;
                    # FIXME: LOGFILE-object as parameter
                    mailit("$fail_email_subject ","$fail_email_body","$fail_email_address");
                }
            }
...
sub start_process_action {
    ...
    my $RTC = $? >> 8;
    if ( $RTC != 0 ) {
        ERROR("process returned with rtc = $RTC");
    } else {
        INFO "Process was OK.";
        # FIXME: good_email mailit();
    }
    return($RTC);
}
